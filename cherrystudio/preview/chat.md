...
![](<../../.gitbook/assets/屏幕截图 2025-03-26 094907 (2).png>)`新话题` 在当前助手内创建一个新话题。

![](../../.gitbook/assets/图片.png)`上传图片或文档` 上传图片需要模型支持，上传文档会自动解析为文字作为上下文提供给模型。

![](<../../.gitbook/assets/图片 (1).png>)`开启网络搜索` 须在设置中配置网络搜索API key，搜索结果作为上下文返回给大模型。

![](<../../.gitbook/assets/图片 (2).png>)`知识库` 开启知识库，详见[知识库教程](broken-reference)。

![](<../../.gitbook/assets/图片 (3).png>)`生成图片` 默认不显示，对于支持生成图片的模型（如Gemini），手动点亮后才能生成图片。

{% hint style="info" %}
由于技术原因，您必须手动点亮按钮才能生成图片，该按钮在功能优化后会移除。
{% endhint %}

![](<../../.gitbook/assets/图片 (5).png>)`选择模型` 对于接下来的对话，切换成指定的模型，保留上下文。

![](<../../.gitbook/assets/图片 (6).png>)`清空消息` 删除该话题下所有内容。

![](<../../.gitbook/assets/图片 (7).png>)`展开` 让对话框变得更大，以便输入长文。

![](<../../.gitbook/assets/图片 (8).png>)`清除上下文` 在不删除内容的情况下，截断模型能获得的上下文。
